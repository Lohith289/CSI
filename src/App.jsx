...
import { Trophy, Zap, Play, RotateCcw, Settings, Award } from 'lucide-react';
...
export default function App() {
  ...
  }, [activeTeam, activePowerUp, challengeQuestion, currentTurnIndex]);

  // === NO ESCAPE RESULT HANDLER ===
  const handleNoEscapeResult = (result) => {
    const { isCorrect } = result;
    const { points, activatingTeam } = noEscapeData;
    let posChange = 0;
  ...
  const handleSubmitAnswer = (result) => {
    const { isCorrect, isChallenged: wasChallenged } = result;
    const stageRules = getStageRules();
          ...
          <div className="board-launch-card">
            ...
            <div className="active-turn-section">
              ...
              <div className="turn-sub">
                {stageRules.difficulty.toUpperCase()} • {stageRules.points} PTS
              </div>
              ...
              <div className="active-team-powerups">
                ...
                {activeTeamObj.powerUps.challenge ? (
                  ...
                  <span className="no-powerups-msg">⚔️ Used</span>
                )}
                ...
                {stageRules.difficulty !== 'easy' ? (
                  activeTeamObj.powerUps.timeBomb ? (
                    ...
                    <span className="no-powerups-msg">💣 Used</span>
                  )
                ) : (
                  <span className="no-powerups-msg">💣 Med/Hard only</span>
                )}
              </div>
            ...
            <div className="opponent-action-section">
              ...
              <div className="pre-question-powerups">
                {activePowerUp ? (
                  ...
                  <>
                    {/* No Escape — opponent can force active team to answer their question */}
                    {opposingTeamObj.powerUps.noEscape ? (
                      <span className="no-powerups-msg">🚫 No Escape available (use during question)</span>
                    ) : (
                      <span className="no-powerups-msg">🚫 No Escape Used</span>
                    )}
                  </>
                )}
              </div>
